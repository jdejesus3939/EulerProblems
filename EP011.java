...
public class EP011 {
	...
	public boolean isInBoundary(int x, int y, int adjCompare) {
		if ((x >= adjCompare) && (x <= matrix[y].length - adjCompare)
				&& (y >= adjCompare) && (y <= matrix.length - adjCompare))
			return true;
		else
			return false;
	}

	public long checkDiagonalDownRight(int x, int y, int adjCompare) {
		long prod = 1;

		if (isInBoundary(x, y, adjCompare)) {
			for (int i = 0; i < adjCompare; i++) {
				prod *= matrix[y + i][x + i];
			}
	...
	public long checkDiagonalDownLeft(int x, int y, int adjCompare) {
		long prod = 1;
	...
	public long checkVertical(int x, int y, int adjCompare) {
		long prod = 1;
	...
	public long checkHorizontal(int x, int y, int adjCompare) {
		long prod = 1;
	...
	public void solution() {
		...
		for (int y = 0; y < matrix.length; y++) {
			for (int x = 0; x < matrix[y].length; x++) {
				maxProduct = Math.max(checkHorizontal(x, y, adjCompare),
						maxProduct);
				maxProduct = Math.max(checkVertical(x, y, adjCompare),
						maxProduct);
				maxProduct = Math.max(checkDiagonalDownLeft(x, y, adjCompare),
						maxProduct);
				maxProduct = Math.max(checkDiagonalDownRight(x, y, adjCompare),
						maxProduct);
			}
		}
